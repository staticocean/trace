...
static void PrintSourceTextErrorLine(PICOC_IOFILE *Stream, const char *FileName,
...
#if defined(UNIX_HOST) || defined(_WIN32)
...
void PrintSourceTextErrorLine(PICOC_IOFILE *Stream, const char *FileName,
							  const char *SourceText, int Line, int CharacterPos)
{
    int LineCount;
    int CCount;
...
{
    PICOC_IOFILE *Stream = Parser->pc->CStdOut;
...
void PlatformPrintf(PICOC_IOFILE *Stream, const char *Format, ...)
{
    va_list Args;
...
void PlatformVPrintf(PICOC_IOFILE *Stream, const char *Format, va_list Args)
{
    const char *FPos;
