...
inline uint8_t trj_proc_fps_update(s_trj_proc_fps *self, s_trj_obj *obj, uint32_t offset)
{
	...
	else  				{ vl_mcopy(&self->rot[0][0], &obj->log_list[offset].rot[0][0]); }
	
	if (offset >= 0x01) { vl_mcopy(&self->rot[1][0], &obj->log_list[offset-1].rot[0][0]); }
	else  				{ vl_mcopy(&self->rot[1][0], &obj->log_list[offset].rot[0][0]); }
	
	vl_mcopy(&self->pos[2][0], &obj->log_list[offset].rot[0][0]);
	
	if (offset <= obj->log_offset-2) { vl_mcopy(&self->rot[3][0], &obj->log_list[offset+1].rot[0][0]); }
	else  							 { vl_mcopy(&self->rot[3][0], &obj->log_list[offset].rot[0][0]); }
	
	if (offset <= obj->log_offset-3) { vl_mcopy(&self->rot[4][0], &obj->log_list[offset+2].rot[0][0]); }
	else  							 { vl_mcopy(&self->rot[4][0], &obj->log_list[offset].rot[0][0]); }
	...
	vl_msumm(rot, rot, &self->rd1[0][0], +8.0 / (12.0 * h));
	vl_msumm(rot, rot, &self->rd1[1][0], -1.0 / (12.0 * h));
